...
y_t = y0;
theta_t = theta0;
path_of_store_ekf(:,1)=x_hat;
path_of_store_eif(:,1)=x_hat;
x_hat2 = x_hat;
for i=1:length(t)
    % input for the robot.
    ...
    path_of_store_eif(:,i+1) = x_hat;

    [x_hat2,P] = my_ekf(x_hat2,Sigma_t,mu_v,sigma_v,mu_omega,sigma_omega,sigma_deltaX,sigma_deltaY,delta_t,z_t);
    path_of_store_ekf(:,i+1) = x_hat2;
end

figure(1);
plot(true_path(1,:),true_path(2,:),'k','LineWidth',2);
hold on;
plot(path_of_store_ekf(1,:),path_of_store_ekf(2,:),'r-.','LineWidth',2);
plot(path_of_store_eif(1,:),path_of_store_eif(2,:),'b:','LineWidth',2);
plot(x0,y0,'kp','MarkerSize',12)
grid on;
title('The path of the EIF of the robot.');
legend({'The true path','The measurement path of ekf','The path of the eif','The start point'},'Location','southeast');
% Compute position errors for EKF and EIF
eif_errors = sqrt((path_of_store_eif(1,:) - true_path(1,:)).^2 + ...
                  (path_of_store_eif(2,:) - true_path(2,:)).^2);

ekf_errors = sqrt((path_of_store_ekf(1,:) - true_path(1,:)).^2 + ...
                  (path_of_store_ekf(2,:) - true_path(2,:)).^2);

% Plot error comparison
figure(2);
plot(t, ekf_errors(2:end), 'b', 'LineWidth', 1.5);
hold on;
plot(t, eif_errors(2:end), 'r', 'LineWidth', 1.5);
xlabel('Time (s)');
ylabel('Position Error (m)');
legend({'EKF Error', 'EIF Error'}, 'Location', 'best');
title('Position Error: EKF vs EIF');
grid on;